...
contract Chat_V0 {
    ...
    mapping(address => avatar) avatarUser;
    ...
    function setAvatar(address _contractAddr, uint256 _tokenId) external {
        require(checkUserExists(msg.sender), "Create an account first!");
        avatarUser[msg.sender].contractAddr = _contractAddr;
        avatarUser[msg.sender].tokenId = _tokenId;
    }

    function getAvatar(address _user) external view returns(address contractAddr, uint256 tokenId) {
        // require(checkUserExists(msg.sender), "Create an account first!");
        return (avatarUser[_user].contractAddr, avatarUser[_user].tokenId);
    }
}